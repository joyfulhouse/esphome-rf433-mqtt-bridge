...
class RFBridgeComponent : public uart::UARTDevice, public Component {
 public:
  ...
  template<typename F> void add_on_bucket_received_callback(F &&callback) {
    this->bucket_data_callback_.add(std::forward<F>(callback));
  }
 ...
 protected:
  ...
  CallbackManager<void(std::string)> bucket_data_callback_;
};
...
template<typename... Ts> class RFBridgeLearnAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...) { this->parent_->learn(); }
...
template<typename... Ts> class RFBridgeStartAdvancedSniffingAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...) { this->parent_->start_advanced_sniffing(); }
...
template<typename... Ts> class RFBridgeStopAdvancedSniffingAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...) { this->parent_->stop_advanced_sniffing(); }
...
template<typename... Ts> class RFBridgeStartBucketSniffingAction : public Action<Ts...> {
 public:
  ...
  void play(const Ts &...) { this->parent_->start_bucket_sniffing(); }
